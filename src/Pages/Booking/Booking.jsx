...
const Booking = () => {
    ...
    useEffect(() => {
      ...
      if (eventItems) {
        ...
        const total=eventItems.reduce((preEvent,currentEvent) => preEvent+currentEvent.price,0)
          ...
          <div className="max-w-screen-2xl mx-auto px-6 md:px-8 lg:px-20 py-4 my-9 ">
            {BookingEvent.length > 0 && (
              ...
              <div className="flex justify-between items-center w-full h-auto bg-amber-500 p-4 mt-2 mb-12 rounded-xl">
                <Link to={'/'}><button className="btn text-white  normal-case font-semibold bg-red-500 hover:bg-black border-red-700 block ">Back</button></Link>

              <h2 className="text-2xl text-white font-semibold">Total Price: {totalService}$</h2>
